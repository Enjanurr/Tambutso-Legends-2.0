...
public class IntroOverlay {
    ...
    public void mousePressed(MouseEvent e) {
        // Allow clicking during FADE_IN or VISIBLE
        ...
        if (nextBtn.getBounds().contains(e.getX(), e.getY())) {
            ...
            System.out.println("[IntroOverlay] Button pressed");
        }
    }

    public void mouseReleased(MouseEvent e) {
        // Allow clicking even during FADE_IN (but not during FADE_OUT)
        if (!open || fadeState == FadeState.FADE_OUT || clickCooldown > 0) {
            nextBtn.resetBools();
            return;
        }

        Rectangle btnBounds = nextBtn.getBounds();
        if (btnBounds.contains(e.getX(), e.getY())) {
            System.out.println("[IntroOverlay] Next clicked - advancing");
