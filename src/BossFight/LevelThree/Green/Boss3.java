...
import BossFight.LevelTwo.MissileProjectile;
public class Boss3 {
    ...
    private BossFight.LevelThree.Green.Boss3.BossState state         = BossFight.LevelThree.Green.Boss3.BossState.FOLLOW;
    private BossFight.LevelThree.Green.Boss3.BossState stateAfterHit = BossFight.LevelThree.Green.Boss3.BossState.FOLLOW;
    ...
    // ── Spawned objects ───────────────────────────────────────
    private final List<MissileProjectile.BossProjectile> bullets = new ArrayList<>();
    private final List<MissileProjectile>    piles   = new ArrayList<>();
    ...
    private void updateStateMachine(float jeepX, float jeepY) {
        ...
        switch (state) {
            ...
            case SKILL1:
                ...
                if (stateTick >= SKILL1_TICKS) enterWait(BossFight.LevelThree.Green.Boss3.BossState.WAIT_AFTER1);
                break;
            ...
            case SKILL2:
                ...
                if (stateTick >= SKILL2_TICKS) enterWait(BossFight.LevelThree.Green.Boss3.BossState.WAIT_AFTER2);
                break;
    ...
    private void enterSkill1() {
        state         = BossFight.LevelThree.Green.Boss3.BossState.SKILL1;
        stateTick     = 0;
        bulletsFired  = 0;
        bulletTick    = 0;
    ...
    private void enterSkill2() {
        state         = BossFight.LevelThree.Green.Boss3.BossState.SKILL2;
        stateTick     = 0;
        pilesLaid     = 0;
        pileTick      = 0;
        s2LoopTick    = 0;
    ...
    private void enterWait(BossFight.LevelThree.Green.Boss3.BossState next) {
        state            = next;
        stateTick        = 0;
        wanderChangeTick = 0;
        wanderInterval   = WANDER_CHANGE_MIN + rng.nextInt(WANDER_CHANGE_MAX - WANDER_CHANGE_MIN);
    ...
    private void enterRandom() {
        state     = BossFight.LevelThree.Green.Boss3.BossState.RANDOM;
        stateTick = 0;
    }

    public void triggerHit() {
        if (state == BossFight.LevelThree.Green.Boss3.BossState.HIT) return;
        stateAfterHit = state;
        state         = BossFight.LevelThree.Green.Boss3.BossState.HIT;
        hitTick       = 0;
        stateTick     = 0;
        aniIndex      = 0;
    }
    ...
    private void fireBullet() {
        ...
        float bulletH = MissileProjectile.BossProjectile.FRAME_H * Game.SCALE;
        ...
        bullets.add(new MissileProjectile.BossProjectile(bx, byCentre, bulletFrames));
    }
    ...
    private void layGarbagePileVertical(int pileIndex) {
        float pileH   = MissileProjectile.PILE_H * Game.SCALE;
        ...
        piles.add(new MissileProjectile(px, py, pileImage));
        pilesLaid++;
    }
    ...
    private void updateAnimation() {
        if (state == BossFight.LevelThree.Green.Boss3.BossState.SKILL2 && currentRow == ROW_SKILL2) return;
    ...
    public void render(Graphics g) {
        for (MissileProjectile p : piles) p.render(g);
        ...
        for (MissileProjectile.BossProjectile b : bullets) b.render(g);
    }
    ...
    public List<MissileProjectile.BossProjectile> getBullets()      { return bullets; }
    public List<MissileProjectile>    getGarbagePiles() { return piles;   }
