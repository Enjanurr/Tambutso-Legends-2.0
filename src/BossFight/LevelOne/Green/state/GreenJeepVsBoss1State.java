package BossFight.LevelOne.Green.state;

import BossFight.LevelOne.Green.entity.*;
import BossFight.LevelOne.Green.overlay.*;
import BossFight.LevelOne.Green.projectile.*;
...
import BossFight.LevelOne.shared.GarbagePile;
import java.awt.*;
...
// ── Heal cooldown ← CHANGE from slow ball to heal ──────
    ...
    public GreenJeepVsBoss1State(Game game, Player player, HealthBar healthBar) {
        ...
        obstacleManager = new BossObstacleManager(game);  // ← ADD THIS LINE
        ...
        pauseButton = new PauseOverlayButton(pauseBtnX, pauseBtnY, pauseBtnScale, () -> {
            paused = true;
            System.out.println("[GreenJeepVsBoss1] Pause button clicked");
        });
        buildDeathOverlay();
        buildDefeatOverlay();
