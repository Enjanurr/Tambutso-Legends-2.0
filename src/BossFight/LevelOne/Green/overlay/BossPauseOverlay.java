package BossFight.LevelOne.Green.overlay;

import BossFight.LevelOne.Green.state.*;
...
public class BossPauseOverlay {
    ...
    private final GreenJeepVsBoss1State greenJeepVsBoss1State;
    ...
    public BossPauseOverlay(GreenJeepVsBoss1State greenJeepVsBoss1State) {
        this.greenJeepVsBoss1State = greenJeepVsBoss1State;
        loadBackground();
        createSoundButtons();
        createUrmButtons();
        createVolumeButton();
    ...
    public void mouseReleased(MouseEvent e) {
        if (isIn(e, musicButton)) {
            ...
            if (resumeBtn.isMousePressed())
                greenJeepVsBoss1State.unpause();          // resume

        } else if (isIn(e, restartBtn)) {
            if (restartBtn.isMousePressed())
                greenJeepVsBoss1State.fullReset();        // full boss fight restart

        } else if (isIn(e, menuBtn)) {
            if (menuBtn.isMousePressed()) {
                greenJeepVsBoss1State.unpause();
                greenJeepVsBoss1State.getPlayer().setBossMode(false);  // Reset boss mode
                greenJeepVsBoss1State.getGame().setLastActiveGameState(GameStates.GREEN_JEEP_VS_BOSS1);
    ...
    private AudioPlayer getAudioPlayer() {
        return greenJeepVsBoss1State.getGame().getAudioPlayer();
    }
}
