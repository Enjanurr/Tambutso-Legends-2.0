package BossFight.LevelTwo.Green.state;

import BossFight.LevelTwo.Green.entity.*;
import BossFight.LevelTwo.Green.overlay.*;
import BossFight.LevelTwo.Green.projectile.*;
...
import BossFight.LevelTwo.Green.projectile.GreenJeepProjectile;
...
// ── Heal cooldown ← CHANGE from slow ball to heal ──────
    ...
    public GreenJeepVsBoss2State(Game game, Player player, HealthBar healthBar) {
        ...
        pauseButton = new PauseOverlayButton(pauseBtnX, pauseBtnY, pauseBtnScale, () -> {
            paused = true;
            System.out.println("[GreenJeepVsBoss2] Pause button clicked");
        });
        buildDeathOverlay();
        buildDefeatOverlay();
        bossBar = new BossHealthBar(BossHealthBar.LifeBarType.BOSS2);
        bossBanner = new BossBanner(2);
...
// ── Heal animation update ← ADD THIS ────────────────────────
        ...
        obstacleManager.checkCollision(jeepHB, this::handleJeepHit);        // Player bullets → boss
